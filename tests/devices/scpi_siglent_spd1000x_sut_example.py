...
"""Example Siglent SPD1000X SCPI power supply SUT client used by integration tests."""
...
class ScpiSiglentSpd1000xSUTExample:
    """Very small helper bridging TCP sockets with SPD1000X SCPI commands."""
    ...
    def select_output(self) -> None:
        self.write(":INSTrument CH1")

    def selected_output(self) -> str:
        return self.query(":INSTrument?")

    def set_voltage(self, value: float) -> None:
        self.write(f"CH1:VOLTage {value}")

    def voltage_setpoint(self) -> float:
        return float(self.query("CH1:VOLTage?"))

    def set_current(self, value: float) -> None:
        self.write(f"CH1:CURRent {value}")

    def current_setpoint(self) -> float:
        return float(self.query("CH1:CURRent?"))

    def output_on(self) -> None:
        self.write(":OUTPut CH1,ON")

    def output_off(self) -> None:
        self.write(":OUTPut CH1,OFF")

    def system_status(self) -> int:
        return int(float(self.query(":SYSTem:STATus?")))

    def measure_voltage(self) -> float:
        return float(self.query(":MEASure:VOLTage? CH1"))

    def measure_current(self) -> float:
        return float(self.query(":MEASure:CURRent? CH1"))

    def measure_power(self) -> float:
        return float(self.query(":MEASure:POWEr? CH1"))
...
__all__ = ["ScpiSiglentSpd1000xSUTExample"]

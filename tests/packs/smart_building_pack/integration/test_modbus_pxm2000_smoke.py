...
"""Smoke test for the Eaton Power Xpert Meter 2000 (PXM2000) Modbus model."""
...
INSTANCE_KEY = "spx_energy_meter_pxm2000_modbus"
MODEL_ID = "Energy.EnergyMeterEatonPxm2000.Modbus"
...
class TestModbusPxm2000Smoke(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def setUp(self):
        ...
        if instance is None:  # pragma: no cover - defensive
            self.skipTest("PXM2000 instance not initialised")
    ...
    def _connect_client(self):
        ...
        if not wait_for_condition(client.connect, timeout=5.0, interval=0.2):
            self.skipTest(
                f"Modbus server not reachable at 127.0.0.1:{self._modbus_port} "
                f"(unit {self._modbus_unit_id})"
            )
        return client

    def test_reads_total_active_power_register(self):
        ...
        try:
            ...
            if result is None or getattr(result, "isError", lambda: True)():
                self.fail("Modbus read_holding_registers failed for PXM2000 active power total")
            value = _decode_float_abcd(result.registers)
            self.assertTrue(
                value == value,  # NaN check
                "Decoded active power total value is NaN",
            )
        finally:
            client.close()

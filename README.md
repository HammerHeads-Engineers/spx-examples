...
you choose a work mode, not just a folder shape:
...
default so the runtime `server_*` tools are available immediately in Codex. Use
an explicit read-only setup only when you want inspection-only MCP access.
...
`.codex/workspace_mode.toml` as a workspace-local override and
`.spx-mcp-workspace.json` as technical metadata. Agents should prefer explicit
user intent first, then the local mode file, then workspace metadata, and only
then fall back to `repo_dev`.
...
- `SPX MCP Setup.app` creates `~/Documents/SPX Codex Workspace`, prepares a
  ...
  workspace also gets a local `.codex/workspace_mode.toml` so agents can tell

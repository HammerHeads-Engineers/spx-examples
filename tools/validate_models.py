        ...
        if isinstance(communication, list):
            for idx, block in enumerate(communication):
                if not isinstance(block, dict) or not block:
                    result.add(f"{path}: communication[{idx}] must be a non-empty mapping")
    ...
    for scenario_name, scenario_def in scenarios.items():
        ...
        if display_name is not None:
            if not isinstance(display_name, str) or not display_name.strip():
                result.add(
                    f"{path}: scenario '{scenario_name}' has empty display_name"
                )
        ...
        if scenario_description is not None:
            if not isinstance(scenario_description, str) or not scenario_description.strip():
                result.add(
                    f"{path}: scenario '{scenario_name}' has empty description"
                )

        overrides = scenario_def.get("overrides")
        if overrides is not None and not isinstance(overrides, dict):
            result.add(f"{path}: scenario '{scenario_name}' overrides must be a mapping")
        ...
        if scenario_conditions is not None:
            if not isinstance(scenario_conditions, list):
                result.add(f"{path}: scenario '{scenario_name}' conditions must be a list")
            else:
    ...
    for entry in models:
        ...
        for list_key in ("protocols", "services", "packages", "profiles"):
            ...
            if not isinstance(value, list):
                result.add(f"{catalog_path}: model entry '{entry_id}' missing '{list_key}' list")
